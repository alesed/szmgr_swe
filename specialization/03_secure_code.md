...
2. funkce ma svuj buffer a za nim navratovou hodnotu (respektive return address)
3. pokud velikost inputu (respektive velikost adresace bufferu) prepiseme, tzn vcetne bufferu prepiseme i return address
4. muzeme spustit kod na jine lokaci (na lokaci odkazovane z prepsane return address)
5. **2 moznosti:**
   1. spustit existujici kod
   2. spusit svuj kod - **jak?**
      1. do bufferu funkce vlozime svuj vlastni kod, ktery chceme executnout
      2. return address prepiseme, aby referencovala zacatek bufferu
      3. return address executne kod v ramci bufferu
...
- checkovani memory boundaries (sizeof)
- pouzivani memory allocation safe jazyku (takze nejlepe zadne C nebo C++)
- randomizace pameti
